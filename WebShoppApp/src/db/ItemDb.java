...
public class ItemDb {
    public static void addNewItem(Item item) throws SQLException {
        ...
        String query = "INSERT INTO Items (item_name, description, price, quantity, category) VALUES (?, ?, ?, ?, ?)";
    ...
    public static Item getItemById(int itemId) throws SQLException {
        Connection connection = DatabaseConnection.getConnection();
        String query = "SELECT item_name, description, price, quantity, category FROM items WHERE item_id = ?";
        try (PreparedStatement statement = connection.prepareStatement(query)) {
